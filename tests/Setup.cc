...
std::string createTestTable() {
    // If this was real code, it might open a connection to a database, create
    // a table with a random name, and return the table name.
    return "test_data_01";
}

void dropTestTable(std::string_view /* name */) {
    // Real code would use the name to drop the table.
}
...
TEST("Test will run multiple setup and teardown code") {
    ...
    updateTestEntry(entry1.id(), "abc");
    updateTestEntry(entry2.id(), "def");
}
...
TEST_SUITE("Test part 1 of suite", "Suite 1") {
    // If this was a project test, it could use the table names from gTable1
    // and tTable2.
...
TEST_SUITE_EX("Test part 2 of suite", "Suite 1", int) {
    // If this were a project test, it could use the table names from gTable1
    // and gTable2
    throw 1;
}
